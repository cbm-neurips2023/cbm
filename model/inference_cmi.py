...
class InferenceCMI(Inference):
    ...
    def extract_masked_state_feature(self, masked_feature, full_state_feature):
        ...
        if self.continuous_state:
            x = masked_feature.T.unsqueeze(dim=-1)                          # (feature_dim, bs, 1)
        else:
    ...
    def predict_from_sa_feature(self, sa_feature, residual_base, abstraction_mode=False):
        """
    ...
    def forward_with_feature(self, feature, actions, mask=None,
        ...
        :param feature:
            ...
            notice that bs can be a multi-dimensional batch size
        :param actions: (bs, num_pred_steps, action_dim) if self.continuous_action else (bs, num_pred_steps, 1)
            notice that bs can be a multi-dimensional batch size
        :param mask: (bs, feature_dim, feature_dim + 1),
        ...
        for i, action in enumerate(actions):
            ...
            full_feature = self.sample_from_distribution(full_dist)
            masked_feature = self.sample_from_distribution(masked_dist)
            causal_feature = self.sample_from_distribution(causal_dist)
    ...
    def update(self, obs, actions, next_obses, eval=False):
        ...
        if self.causal_opt_freq and self.update_num % self.causal_opt_freq == 0:
            pred_next_dist = pred_next_dist
        else:
    ...
    def reward(self, obs, actions, next_obses, output_numpy=False):
        ...
        :param actions: (bs, num_pred_steps, action_dim) during policy training or (action_dim,) after env.step()
        :param next_obses: (bs, num_pred_steps, obs_spec) during policy training or (obs_spec,) after env.step()
        :param output_numpy: output numpy or tensor
        :return: (bs, num_pred_steps,) or scalar
        """
        ...
        with torch.no_grad():
            ...
            # (bs, num_pred_steps), (bs, num_pred_steps)
            ...
            pred_diff_reward = full_log_prob - causal_log_prob                  # (bs, num_pred_steps)
            ...
            reward = reward[..., None]                                          # (bs, num_pred_steps, 1)
    ...
    def eval_prediction(self, obs, actions, next_obses):
        ...
        return feature, next_feature, pred_next_dist.mean
