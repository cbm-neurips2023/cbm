...
class GumbelMatrix(torch.nn.Module):
    ...
    def forward(self, bs, tau=1, drawhard=True):
        if self.training:
            sample = gumbel_sigmoid(self.log_alpha, bs, tau=tau, hard=drawhard)
        else:
...
class GumbelPartition(torch.nn.Module):
    """
    Random matrix M used for the mask. Can sample a matrix and backpropagate using the
    Gumbel straigth-through estimator.
    """
    def __init__(self, shape, params):
        super(GumbelPartition, self).__init__()
        assert len(shape) == 2
        self.num_partitions = shape[-1]
        self.tau = 1
        self.params = params
    ...
    def setup_annealing(self, step):
        starts = self.params.training_params.init_step
        ends = self.params.training_params.total_step
        start_val = 5
        end_val = 0.05
        coef = np.clip((step - starts) / (ends - starts), 0, 1)
        self.tau = start_val + coef * (end_val - start_val)

    def forward(self, bs, tau=None, hard=True):
        ...
        if self.training:
            if tau is None:
                tau = self.tau
            log_alpha = self.log_alpha.expand(*bs, *self.shape)
            sample = F.gumbel_softmax(log_alpha, tau=tau, hard=hard, dim=-1)
        else:
            log_alpha_max, _ = self.log_alpha.max(dim=-1, keepdim=True)
            sample = (self.log_alpha == log_alpha_max).float()
            sample = sample.expand(*bs, *self.shape)
        return sample

    def get_prob(self):
        """Returns probability of getting one"""
        return F.softmax(self.log_alpha, dim=-1)
