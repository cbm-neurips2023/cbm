...
class BaseSkill:
    def __init__(self, skill_params):
        ...
        if "ToolUse" in env_name:
            ...
            causal_params = env_params.manipulation_env_params.causal_env_params
            num_movable_objects = causal_params.num_movable_objects
            num_unmovable_objects = causal_params.num_unmovable_objects
            num_random_objects = causal_params.num_random_objects
            obj_name_mapper.update({i: "mov{}".format(i)
                                    for i in range(num_movable_objects)})
            obj_name_mapper.update({i + num_movable_objects: "unmov{}".format(i)
                                    for i in range(num_unmovable_objects)})
            obj_name_mapper.update({i + num_movable_objects + num_unmovable_objects: "rand{}".format(i)
                                    for i in range(num_random_objects)})
            num_objs = num_movable_objects + num_unmovable_objects + num_random_objects
