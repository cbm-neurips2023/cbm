...
class InferenceNPS(Inference):
    def __init__(self, encoder, decoder, params):
        ...
        super(InferenceNPS, self).__init__(encoder, decoder, params)
    ...
    def init_model(self):
        ...
        self.num_rules = num_rules = nps_params.num_rules
        ...
        self.rule_embed = nn.Parameter(torch.randn(num_rules, 1, rule_embed_dim, dtype=torch.float32))
        feature_embed_dim = nps_params.feature_fc_dims[-1]
        self.rule_selector = Attention(feature_dim, feature_embed_dim, num_rules, rule_embed_dim,
        ...
        for out_dim in nps_params.rule_fc_dims:
            self.rule_weights.append(nn.Parameter(torch.zeros(num_rules, in_dim, out_dim)))
            self.rule_biases.append(nn.Parameter(torch.zeros(num_rules, 1, out_dim)))
            in_dim = out_dim

        if continuous_state:
            self.rule_weights.append(nn.Parameter(torch.zeros(num_rules, in_dim, 2)))
            self.rule_biases.append(nn.Parameter(torch.zeros(num_rules, 1, 2)))
        else:
            for feature_i_dim in self.feature_inner_dim:
                final_dim = 2 if feature_i_dim == 1 else feature_i_dim
                self.rule_last_layer_weights.append(nn.Parameter(torch.zeros(num_rules, in_dim, final_dim)))
                self.rule_last_layer_biases.append(nn.Parameter(torch.zeros(num_rules, 1, final_dim)))
    ...
    def forward_step(self, feature, action):
        ...
        rule_embed = self.rule_embed.repeat(1, bs, 1)                       # (num_rules, bs, rule_embed_dim)
        rule_logits = self.rule_selector(state_feature, rule_embed)         # (bs, feature_dim, num_rules)
        cond_logits = self.cond_selector(state_feature, sa_feature)         # (bs, feature_dim, feature_dim + 1)
        if self.training:
            rule_select = F.gumbel_softmax(rule_logits, hard=True, dim=-1)  # (bs, feature_dim, num_rules)
            cond_select = F.gumbel_softmax(cond_logits, hard=True, dim=-1)  # (bs, feature_dim, feature_dim + 1)
        else:
            # (bs, feature_dim, num_rules)
        ...
        # (num_rules, feature_dim, bs, 2 * feature_embed_dim)
        rule_input = rule_input.repeat(num_rules, 1, 1, 1)

        # (num_rules, bs * feature_dim, 2 * feature_embed_dim)
        rule_input = rule_input.view(num_rules, bs * feature_dim, -1)

        # (num_rules, bs * feature_dim, out_dim)
        x = forward_network(rule_input, self.rule_weights, self.rule_biases)
        x = x.view(num_rules, feature_dim, bs, -1)                           # (num_rules, feature_dim, bs, out_dim)

        rule_select = rule_select.permute(2, 1, 0)                          # (num_rules, feature_dim, bs)
        rule_select = rule_select.unsqueeze(dim=-1)                         # (num_rules, feature_dim, bs, 1)
        ...
        if self.continuous_state:
            ...
            x = F.relu(x)                                                   # (num_rules, feature_dim, bs, out_dim)
            x = torch.unbind(x, dim=1)                                      # [(num_rules, bs, out_dim)] * feature_dim
            # [(num_rules, bs, feature_i_inner_dim)] * feature_dim
            x = forward_network_batch(x,
            ...
            rule_select = rule_select.unbind(dim=1)                         # [(num_rules, bs, 1)] * feature_dim
