...
class InferenceMLP(Inference):
    def __init__(self, encoder, target_encoder, params):
        self.mlp_params = params.inference_params.mlp_params
        super(InferenceMLP, self).__init__(encoder, target_encoder, params)
    ...
    def forward_step(self, feature, action):
        ...
        if self.continuous_state:
            ...
            if self.residual:
                mu = mu + feature                                                       # (bs, feature_dim)

            if self.training:
                sample = reparameterize(mu, log_std)                                    # (bs, feature_dim)
            else:
                sample = mu
            ...
            sample, dist = [], []
            for feature_i, feature_i_inner_dim, dist_i in zip(feature_copy, self.feature_inner_dim, raw_dist):
                if feature_i_inner_dim == 1:
                    mu, log_std = torch.split(dist_i, 1, dim=-1)  # (bs, feature_dim) * 2
                    log_std = torch.clip(log_std, min=self.log_std_min, max=self.log_std_max)
                    if self.residual:
                        mu = mu + feature_i  # (bs, feature_dim)

                    if self.training:
                        sample_i = reparameterize(mu, log_std)  # (bs, feature_dim)
                    else:
                        sample_i = mu
                    dist_i = (mu, log_std)
                else:
                    if self.training:
                        # [(bs, feature_i_dim)] * feature_dim
                        sample_i = F.gumbel_softmax(dist_i, hard=True)
                    else:
                        # [(bs, feature_i_dim)] * feature_dim
                        sample_i = F.one_hot(torch.argmax(dist_i, dim=-1), dist_i.size(-1))
